...
angular.module('santedb').controller('JobAdminController', ["$scope", "$rootScope", "$timeout", function ($scope, $rootScope, $timeout) {
    ...
    $scope.renderState = function (job) {
        switch (job.state) {
            ...
            case "Aborted":
                return `<span class="badge badge-danger badge-pill"><i class="fas fa-exlamation-triange"></i> ${SanteDB.locale.getString("ui.state.abort")}</span><br/><small>{{ '${(job.status ||  '').replaceAll("'","\\'")}' | leftEllipsis: 30 }}</small>`;
            case "Stopped":
                return `<span class="badge badge-danger badge-pill"><i class="fas fa-exlamation-triange"></i> ${SanteDB.locale.getString("ui.state.stop")}</span><br/><small>{{ '${(job.status ||  '').replaceAll("'","\\'")}' | leftEllipsis: 30 }}</small>`;
            default:
    ...
    $scope.runJob = async function (id, index, jobParameters) {
        ...
        if (confirm(SanteDB.locale.getString("ui.emr.job.runJob.confirm"))) {
            try {
                var parms = {};
                if (jobParameters) {
                ...
                toastr.success(SanteDB.locale.getString("ui.emr.job.runJob.success", { job: jobInfo.name }));
                $("#jobsTable table").DataTable().ajax.reload();
    ...
    $scope.cancelJob = async function (id) {
        if (confirm(SanteDB.locale.getString("ui.emr.job.cancelJob.confirm"))) {
            try {
